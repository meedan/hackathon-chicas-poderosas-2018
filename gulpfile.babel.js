...
gulp.task('config:build:chrome', () => {
  ...
  gulp.src('./src/chrome/extension/manifest.json.example')
  .pipe(replace('Title', 'Hackathon'))
  .pipe(replace('Description', 'Candidatos brasileiros em 2014'))
...
gulp.task('build:android', shell.task(
  'cd src/android && npm install && rm -fr android/app/build.gradle android/settings.gradle stylesheet.js android/app/src/main/java/com/hackathon/MainActivity.java index.android.js android/app/src/main/AndroidManifest.xml android/app/src/main/res/mipmap* && react-native android && cp index.js index.android.js && cp AndroidManifest.xml android/app/src/main/ && cp MainActivity.java android/app/src/main/java/com/hackathon/ && cp -r ../assets/img/logo/android/* android/app/src/main/res/ && cp ../../build/android/css/stylesheet.js . && cp build.gradle android/app && cp settings.gradle android/ && react-native run-android && npm start && cd -'
));
